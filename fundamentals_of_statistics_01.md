...
- [2. Сравнение средних](#2-сравнение-средних)
  - [2.1. T-распределение](#21-t-распределение)
  - [2.2. Сравнение двух средних; t-критерий Стьюдента](#22-сравнение-двух-средних-t-критерий-стьюдента)
  - [2.3. Проверка распределения на нормальность, QQ-Plot](#23-проверка-распределения-на-нормальность-qq-plot)
  - [2.4. Однофакторный дисперсионный анализ](#24-однофакторный-дисперсионный-анализ)
  - [2.5. Множественные сравнения в ANOVA](#25-множественные-сравнения-в-anova)
  - [2.6. Многофакторный ANOVA](#26-многофакторный-anova)
  - [2.7. АБ тесты и статистика](#27-аб-тесты-и-статистика)
...
- **Качественные:**
    - **Номинальные** – переменная, используемая для разделения всех объектов выборки на определенные группы. Используются как имена групп (например, 1-женщины, 2-мужчины);
    - **Ранговые** – переменная, которая позволяет ранжировать объекты выборки. Возможны только сравнения (например результаты забега, т.е. бегун с рангом 1 быстрее бегуна с рангом 5, но мы не можем сказать ни во сколько, ни на сколько выражено его преимущество).
...
**Мода** (Mode) – значение измеряемого признака, которое встречается максимально часто. В выборке может быть одна или несколько мод.
...
**Box plot** – вид диаграммы, показывающий в удобной форме медиану, нижний и верхний квартили, минимальное и максимальное значение выборки, выбросы.

![boxplot](res/boxplot.png)

Квартили и inter quartule range (IQR) используют, чтобы оценить наличие выбросов. Алгоритм расчета – посчитали квартили, посчитали разницу между ними, вычислили теоретический максимум и минимум, сравнили с имеющимися и выяснили есть ли у нас выбросы и сколько их.
...
**Z-преобразование** (или стандартизация) (Z-transform) – преобразование полученных данных в стандартную Z-шкалу (Z-scores) со средним значением $M_{z}=0$ и дисперсией $D_{z}=1$. Чтобы привести к такому виду необходимо из каждого наблюдения $x_{i}$ отнять среднее значение $\overline{x}$ и разделить на стандартное отклонение $\mathrm{sd}$
...
Зная стандартное отклонение генеральной совокупности и размер выборки мы можем рассчитать стандартную ошибку среднего:
...
**p-уровень значимости** – это вероятность получать такие или боле выраженные различия при условии, что в генеральной совокупности никаких различий на самом деле нет.
